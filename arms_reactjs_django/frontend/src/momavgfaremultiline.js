...
class MOMAvgFareMultiline extends Component {
    constructor(props) {
        super(props);
        this.state = { 
            momavgeFareMultiline: [],
        isLoading: true,
        };
    }
    ...
    componentDidMount() {
        ...
        apiServices.getMOMAvgFareMultilineChart().then( (momavgfare_multilineData) => {
          const avgfareName = "MoM Avgfare";
          this.setState({ momavgeFareMultiline: momavgfare_multilineData,isLoading: false});
          this.avgFareLine(this.state.momavgeFareMultiline , avgfareName);
        });
    
    }

    avgFareLine(data, name) {
        ...
        var svg = d3.select("#avgfare_multiline").append("svg")
        ...
        segment.selectAll("dot")
            ...
            .on("mousemove", function(d) {
                divToolTip
                    .text(this.parentNode.__data__.key + " : " + window.numberFormat(d.AvgFare, 2)) // here we using numberFormat function from Dashboard-indicator.js
    ...
    render() {
        return (
            <div id="avgfare_multiline" style={{ "padding": "5px" }}></div>
        )
    }
}

export default MOMAvgFareMultiline;